...
class Blogger extends React.Component {
    ...
    loadCommentsFromServer(tagId, all) {
        ...
        $.ajax({
            url: netService.getAllList.IPRESOURCE_PAGE,
            data: param,//{ itemsPerPage: 8, offset: this.state.offset, tag: tagId },
            dataType: 'json',
            type: 'GET',
            success: (result) => {
                this.setState({ data: result.data.data, pageCount: Math.ceil(result.data.count / 8.0) });
