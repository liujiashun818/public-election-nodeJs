...
class Comment extends React.Component {
   ...
   handleReplyClick(){
		...
		var options = {
   		...
   		let data = {
   			...
   			comment: this.base64encode(xss(this.state.replyContent, options))
		   }
   		// console.log(data);
   ...
   base64encode(str){
	...
	_createCommentEditor(){
		var commentEl = this.refs.commentEditor;
		if (commentEl != null) {
			...
			this._editor.customConfig.onchange = this.handleReplyChange;
		...
		return(
			<div className="comment">
				<div className="reviewCon">
					<div className="top">
						<div className="portrait" onClick={()=>{
							...
							<div className = "info">
								...
								<div className="meta">{this.formatDate(new Date(comment.time), 'yyyy-MM-dd hh:mm:ss')}</div>
							</div>
						</div>
					...
					<div className="commentContent" style={{marginLeft:"44px",marginTop:"10px",marginBottom:"10px",wordWrap:"break-word"}} dangerouslySetInnerHTML={{__html: this.base64decode(comment.comment)}}></div>
					...
					{this.state.showReply &&
						<div className="review commentRely" style={{width: '100%'}}>
							...
							<div ref="commentEditor" style={{height: '150px', margin: '0px'}}></div>

							<div className="reviewBottom" style={{border: 'none', width: '100%'}}>
								{/* <span className="nickname">昵称</span> */}
								<span className="reviewInfo" style={{height: '27px', lineHeight: '27px'}} onClick={this.handleReplyClick}>回复</span>
							</div>
						</div>
					}			
				</div>
